<template>
    ...
    <div class="post-list-item" :data-id="post.id" v-else >
		<div class="upper-row">
      ...
      <div v-if="viewer" class="post-list-item-header-right">
        ...
        <!-- Service layer guarantees pending posts are returned ONLY for owning user but this still needs to be here inside above conditional-->
          <button v-if="!post.isApproved && post.isApproved !== undefined" style="pointer-events: none; cursor: default;" class="btn btn-default" >{{ t('Awaiting Approval') }}</button>
...
(this cuz of dumb i18n parser...) -->
        ...
        <template v-else-if="(route.id === 'groups_main' && isMember && ['OFFER', 'WANTED', 'LEND', 'BORROW'].includes(postType))">
          <span class="post-list-item-date text-lighten-less">
              {{ post.date | mreldate(post.time, (post.group && post.group.timezone ? post.group.timezone : timezone )) }}
          </span>
        ...
        <template v-else-if="(route.id !== 'groups_main' && ['OFFER', 'WANTED', 'LEND', 'BORROW'].includes(postType))">
          <span class="post-list-item-date text-lighten-less">
              {{ post.date | mreldate(post.time, (post.group && post.group.timezone ? post.group.timezone : timezone )) }}
          </span>
          <fc-messages-detail-input :subject="t('Reply to your post') + ': ' + post.subject" topic-type="post" :topic-id="String(post.id)" :custom-trigger="replyButton">
