				...
				<div v-if="viewer" class="post-grid-item-header-right">
					...
					<template v-else-if="viewer === post.userId">
						...
						<select class="manage-post-select post-grid-select" :class="`btn-${lowercase(postType)}`" v-on:change="manageOp">
							...
							<option v-if="closedType" value="mark" >{{ t('Mark As') }} {{ t(closedType)|firstupper }}) }}</option>
					...
					<fc-messages-detail-input v-else-if="((route.id === 'groups_main' && isMember ) || !['groups_main','search_posts'].includes(route.id) && ['OFFER', 'WANTED', 'LEND', 'BORROW'].includes(postType))" :subject="t('Reply to your post') + ': ' + post.subject" topic-type="post" :topic-id="post.id" :custom-trigger="replyButton">
